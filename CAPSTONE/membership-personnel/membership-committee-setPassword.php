...
<body>
    ...
    <div id="page-content" class="content">
        <nav class="navbar navbar-expand-lg" id = "side-navbar">
            ...
            <span class="navbar-brand mb-0 h1 text-white">Set Password</span>
            ...
            <div class="container-xxl mb-5">
                <div class = "table-responsive">
                    <div class = "row mt-3 mb-4">
                        <div class = "col-6 input-group" style = "width: 400px;">
                            <input type = "text" name = "txt_search" class = "form-control border border-dark-subtle" style = "width: 250px; margin-left: 5px;">
                            <input type = "submit" name = "searchBtn" class = "btn btn-outline-secondary border border-dark-subtle" value = "Search">
                        </div>
                    ...
                    <table class="table" id="members-loan-history-table">
                        ...
                        <tbody>
                        <?php
                            $sql = "SELECT * FROM members_db WHERE acc_status = 'Pending' 
                            AND DateTime = 'Registered'";

                            if (isset($_POST['searchBtn'])) {
                                $search = $_POST['txt_search'];
                                $sql = "SELECT application_num, LastName, FirstName, MiddleName, DateTime, acc_status FROM members_db 
                                    WHERE (application_num LIKE '%$search%' 
                                        OR LastName LIKE '%$search%' 
                                        OR FirstName LIKE '%$search%' 
                                        OR MiddleName LIKE '%$search%' 
                                        OR DateTime LIKE '%$search%') 
                                        AND acc_status = 'Pending' 
                                        AND DateTime = 'Registered'";
                            }
                            ...
                            if ($result->num_rows == 0) {
                                ...
                                echo "<td colspan='8' style='text-align:center; background-color: rgba(0, 0, 0, 0.3);'>
                                        <b><p style='font-size: 21px; margin-top: 8%; color: black;'>
                                        <i class='bi bi-info-circle-fill' style='font-size: 18px;'></i>&nbsp;No pending loan request at the moment</p></b>
                                    </td>";
                                echo "</tr>";
                            } else {
                                while ($row = $result->fetch_assoc()) {
                                    echo "<tr id='transaction-tr'>";
                                    echo "<td style='text-align: center;'>" . htmlspecialchars($row['application_num']) . "</td>";
                                    echo "<td>" . htmlspecialchars($row['LastName']) . "</td>";
                                    echo "<td>" . htmlspecialchars($row['FirstName']) . "</td>";
                                    echo "<td style='text-align: center;'>" . htmlspecialchars($row['MiddleName']) . "</td>";
                                    echo "<td style='text-align: center;'>" . htmlspecialchars($row['DateTime']) . "</td>";
                                    echo "<td style='text-align: center; color: #F45C2F;'>" . htmlspecialchars($row['acc_status']) . "</td>";
                                    echo "<td style='text-align: center;'>
                                            <a href='membership-committee-setPasswordForm.php?application_num=" . htmlspecialchars($row['application_num']) . "' class='edit-btn'><i class='bx bxs-edit'></i></a>
                                        </td>";
                                    echo "</tr>";
                                }
                            }
